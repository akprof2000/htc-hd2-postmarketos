...
| **Bluetooth** | заплатка прошивки грузится сама, телефон виден снаружи, сопряжён с ноутбуком |
...
**Что не работает — честно.** Снять питание это ядро не умеет: все три
пути (`poweroff`, SysRq, `poweroff -f`) виснут при остановке драйверов;
глубокий сон входит, но не просыпается. Поэтому кнопка «В сон» делает
ровно то, что написано: гасит экран и держит ядро на сне экрана, из
которого оно будится кнопкой — проверено на кабеле и без. Снимок
камерой изредка вешает телефон.
