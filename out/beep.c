 ...
 * Перечитывает калибровку ACDB, включает маршрут на динамик, играет 660 Гц 2 с.
 * ioctl из include/linux/msm_audio.h этого ядра. */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <string.h>
...
struct msm_audio_config {
	uint32_t buffer_size, buffer_count, channel_count, sample_rate, type, unused[3];
};

int main(int argc, char **argv) {
	/* beep [h] : h = наушники, иначе динамик */
	uint32_t dev = (argc > 1 && argv[1][0] == 'h') ? DEV_HEADSET_STEREO : DEV_SPKR_MONO;
	int ctl = open("/dev/msm_audio_ctl", O_RDWR);
	if (ctl >= 0) {
		/* перечитать калибровку: при первом обращении в загрузке файла ещё не было */
		char fn[64];
		...
		if (ioctl(ctl, AUDIO_REINIT_ACDB, fn) < 0)
			...
			printf("acdb: перечитан\n");
		uint32_t sw[2] = { dev, 0 };
		if (ioctl(ctl, AUDIO_SWITCH_DEVICE, &sw) < 0) perror("SWITCH_DEVICE");
		uint32_t vol = 100;
	...
	printf("буфер %u x%u, каналов %u, частота %u\n",
	       c.buffer_size, c.buffer_count, c.channel_count, c.sample_rate);
	c.channel_count = 2; c.sample_rate = 44100;
	...
	while (t < total) {
		...
		for (i = 0; i < frames; i++, t++) {
			int16_t s = (int16_t)(12000.0 * sin(2*3.14159265*660.0*t/44100.0));
