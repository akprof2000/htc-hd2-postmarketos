...
static const char *TYPING[] = {"xfce4-terminal", "notes", NULL};
...
static double dark_at = 0;
    ...
    for (;;) {
        ...
        if (tfd >= 0) {
            ...
            // сенсор, и следующее нажатие разбудит через wake_full —
            // с отменой просьбы «mem». Спрашивать ядро, висит ли
            // просьба, нельзя: /sys/power/state здесь показывает список
