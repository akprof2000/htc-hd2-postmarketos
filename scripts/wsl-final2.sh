...
cp /mnt/c/Projects/HTC-HD2-T8585/out/06-mdp-24bpp-as-32.patch "$A/"
grep -q '06-mdp-24bpp-as-32' "$A/APKBUILD" || awk '/^\t05-drop-missing-msm_kexec\.patch$/ { print; print "\t06-mdp-24bpp-as-32.patch"; next } { print }' "$A/APKBUILD" > "$A/APKBUILD.new" && mv "$A/APKBUILD.new" "$A/APKBUILD"
